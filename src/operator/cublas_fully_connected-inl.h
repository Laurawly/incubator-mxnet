...
class CuBLASFullyConnectedOp : public Operator {
 ...
 private:
  ...
  static void CublasGemm(mshadow::Stream<gpu> *s, bool transposeA, bool transposeB,
    ...
    // unless the compile-time flag MSHADOW_USE_PASCAL == 0, in which case
    // float32 is chosen in preference to float16.
    auto compute_precision = CublasType<DType>::kCudaFlag;
    if (MSHADOW_USE_PASCAL == 0 &&
        CublasType<DType>::kFlag == mshadow::kFloat16 &&
        algo_precision == -1) {
